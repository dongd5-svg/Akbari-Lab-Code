...
async function load(name) {
  ...
  return AK.readMat(buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength), name);
}
...
const PREF = loadFixture('pipeline_ref');
...
for (const [file, label] of [['sample_v73.mat','v7.3 (HDF5)'], ['sample_v7.mat','v7 (zlib)'], ['sample_v6.mat','v6 (uncompressed)']]) {
  ...
  const t = AK.pickVariable(v, ['time']);
  const m = AK.pickVariable(v, ['mean_data']);
...
console.log('\n--- flat ROI file (v7) ---');
{
  ...
  const got = need.map(n => AK.pickVariable(v, [n]));
...
for (const [file, label] of [['sample_struct_v7.mat','struct (v7)'], ['sample_struct_v73.mat','struct (v7.3)']]) {
  ...
  const h = AK.pickVariable(v, ['hbo2']);
...
console.log('\n--- mixed types (single, int32) ---');
{
  ...
  const sp = AK.pickVariable(v, ['sp']);
  const iv = AK.pickVariable(v, ['iv']);
...
{
  let threw = false;
  try { await AK.readMat(new TextEncoder().encode('x'.repeat(300)).buffer); } catch { threw = true; }
