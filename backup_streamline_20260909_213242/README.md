...
The data download defaults to **Original flow**, preserving every input camera
frame and its recorded timestamp without interpolation, cleaning or normalization.
Choose **Day 1 reference table** to export the first 11,673 original values per
animal in separate columns, matching the supplied Day 1 workbook's data extent.
Column headings identify the animal; column order follows the loaded recordings.
A shorter recording is rejected instead of being padded. For Mouse 287, the
matching full Day 1 recording is `Mouse 287_Baseline_LSI2.mat`.
Choose **Processed flow** for the previous resampled data export. Plots and the
processed summary retain their existing calculations.
