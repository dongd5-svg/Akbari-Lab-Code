...
$('dlData').onclick = () => {
  try {
    const mode = $('dataExport').value;
    const rows = mode === 'reference' ? AK.referenceRows(state.results)
      : mode === 'original' ? AK.originalRows(state.results, state.assign)
      : allTraceRows(state.results, state.assign);
    if (!rows.length) return;
    const prefix = state.results.length === 1 ? state.results[0].id : 'all_animals';
    const name = `${prefix}_${mode}_data.csv`;
