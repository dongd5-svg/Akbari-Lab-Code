...
**Raw data CSV** preserves every input camera frame and its recorded timestamp
without interpolation, cleaning or normalization. There is no fixed row limit.
**Processed data CSV** exports the resampled analysis. **Summary CSV** exports
one processed summary per animal. Plots show processed results.

Data exports default to **Side by side**: each animal has its own time and
measurement columns. Recordings retain their original lengths; cells after a
shorter recording are blank. A shared row does not imply equal timestamps.
**Stacked rows** places animals one below another. **Arrange by group** keeps
assigned groups together and labels side-by-side columns with their group and
animal. Unassigned animals follow under Ungrouped when groups exist. These
layout controls apply to both raw and processed exports, not the summary.
